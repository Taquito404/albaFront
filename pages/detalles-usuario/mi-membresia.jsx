...
const MiMembresia = () => {
    ...
    const createCheckoutSession = async (priceId) => {
        try {
            ...
            const { data } = await axios.post('https://dev-alba.herokuapp.com/stripe/create-checkout-session', { priceId }, options);
            return data
        } catch (error) {
            console.error(error)
        }
    ...
    const handleDeleteSubscription = async () => {
        try {
            ...
            const { data } = await axios.delete(`https://dev-alba.herokuapp.com/stripe/subscriptions/${user.subscriptionId}`, options);
            setCanceledSub(true);
                    ...
                    <div className={miMembresiaStyles.userSubInfoWrapper}>
                        ...
                        <div className={miMembresiaStyles.membresiaWrapper}>
                            ...
                            <div>
                                ...
                                <p>{user.subscriptionId ? 'Pro' : 'Free'} {user.subscriptionId ? <span onClick={showModal}>Cancelar suscripción</span> : <span onClick={handleSubscribe}>Iniciar suscripción</span>}</p>
                            </div>
                        </div>
                    </div>
                </div>
