...
const AgregarLeccion = () => {
    ...
    const handleChangeValues = ev => setLeccion({ ...leccion, [ev.target.name]: ev.target.value })
    ...
    const handleDeleteLeccion = async id => {
        try {
            ...
            const { data } = await axios.delete(`https://dev-alba.herokuapp.com/videos/${id}`, options);
            setHasRemoved(true);
            setHasRemoved(false);
        } catch (error) {
            console.error(error)
    ...
    const handleSubmit = async ev => {
        ...
        try {
            ...
            const { data } = await axios.post('https://dev-alba.herokuapp.com/videos', leccion, options);
                ...
                <div className="container container-fluid">
                    ...
                    <div className="row mt-5">
                        <div className="col-12">
                            <form onSubmit={handleSubmit}>
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-control border rounded shadow-sm p-1" placeholder="E.g. Maternidad IV" name="title" value={leccion.title ? leccion.title : ''} onChange={handleChangeValues} />
                                </div>
                                <div className="form-group">
                                    <label className="font-weight-bold">Descripcion</label>
                                    <input type="text" className="form-control border rounded shadow-sm p-1" placeholder="E.g. En este video se podra visualizar..." name="description" value={leccion.description ? leccion.description : ''} onChange={handleChangeValues} />
                                </div>
                                <div className="form-group">
                        ...
                        <div className="col-12 mt-5">
                            ...
                            {
                                !lecciones ? null : lecciones.map(video => <Leccion video={video} handleDeleteLeccion={handleDeleteLeccion} />)
                            }
                        </div>
                    </div>
                </div>
